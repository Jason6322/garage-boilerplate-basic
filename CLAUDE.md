...
Pages: `/` · `/auth/signin` · `/auth/signup` · `/auth/reset-password` · `/dashboard` · `/notes` · `/team` · `/profile` · `/settings` (route groups `(auth)`, `(dashboard)`). Backend: `GET /api/health` (public); everything else under `/api` requires `Authorization: Bearer <ID token>`.
